...
void Fast3dGui::HandleWindowEvents(Fast::WindowEvent event) {
    switch (mImpl.Backend) {
        ...
        case WindowBackend::FAST3D_SDL_VULKAN: {
            ...
            if (mImpl.Backend == WindowBackend::FAST3D_SDL_METAL && mImpl.Metal.Renderer != nullptr) {
...
void Fast3dGui::CalculateGameViewport() {
    ...
    interpreter->mCurDimensions.width = (uint32_t)(size.x * dpiScale * interpreter->mCurDimensions.internal_mul);
    interpreter->mCurDimensions.height = (uint32_t)(size.y * dpiScale * interpreter->mCurDimensions.internal_mul);
